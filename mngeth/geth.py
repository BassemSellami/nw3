...
h1_start_node = (f"nohup geth --datadir=$DDR1 --networkid {network_id} "
...
h2_start_node = (f"nohup geth --datadir $DDR2 --networkid {network_id} "
                 f"--port {bootnode_port + 1} --bootnodes $ENODE_ADDRESS &")
h2_check_join = "geth attach $DDR2/geth.ipc --exec admin.peers"
h3_start_mine = (f"nohup geth --datadir $DDR3 --networkid {network_id} "
                 f"--port {bootnode_port + 2} --mine --minerthreads=1 --etherbase={miner_base} &")
