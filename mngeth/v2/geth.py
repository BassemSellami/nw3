...
# geth_bin_file = '~/go-ethereum/build/bin/geth'
geth_bin_file = 'geth'
...
node_1_check_blocks_alt = (f"{geth_bin_file} attach $DDR1/geth.ipc "
                           f"--exec 'eth.getBlock(\"latest\")' > ~/nw3/mngeth/data/nohup_node_1_block.out")


def read_get_block(file="./data/nohup_node_1_block.out"):
    with open(file) as f:
