...
class SingleSwitchTopo(Topo):
    ...
    def build(self, k):
        ...
        for h in range(k):
            host = self.addHost(f'h{(h + 1)}', **conf["nw_node_param"])
            self.addLink(host, switch, **conf["nw_link_param"])
...
class SingleSwitchTopoSlowH1(Topo):
    ...
    def build(self, k, h1bw):
        ...
        host = self.addHost('h1', **conf["nw_node_param"])
    ...
    def build(self, k=2, n=1, **_opts):
        ...
        for i in range(1, k + 1):
            ...
            for j in range(1, n + 1):
                host = self.addHost(f'h{(i - 1) * n + j}', **conf["nw_node_param"])
                self.addLink(host, switch, **conf["nw_link_param"])
            # Connect switch to previous
            if lastSwitch:
                self.addLink(switch, lastSwitch, **conf["nw_link_param"])
            lastSwitch = switch
    ...
    def addTree(self, depth, fanout):
        ...
        if isSwitch:
            ...
            for _ in range(fanout):
                ...
                self.addLink(node, child, **conf["nw_link_param"])
        else:
            node = self.addHost('h%s' % self.hostNum, **conf["nw_node_param"])
            self.hostNum += 1
        return node
    ...
    def build(self, c, a, e, h):
        ...
        for core in self.cores:
            for aggr in self.aggrs:
                self.addLink(core, aggr, **conf["nw_link_param"])

        for aggr in self.aggrs:
            for edge in self.edges:
                self.addLink(aggr, edge, **conf["nw_link_param"])

        for edge in self.edges:
            for i in range(h):
                host = self.addHost(next(self.hgen), **conf["nw_node_param"])
                self.host_list.append(host)
                self.addLink(edge, host, **conf["nw_link_param"])
